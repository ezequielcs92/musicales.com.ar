...
export async function generateMetadata({
  params,
}: PageProps<"/entrevistas/[slug]">): Promise<Metadata> {
...
export default async function Nota({ params }: PageProps<"/entrevistas/[slug]">) {
  const { slug } = await params;
  const nota = await traer(slug);
  if (!nota) notFound();

  return <Articulo nota={nota} ruta={RUTA} etiquetaSeccion="Entrevistas" />;
}
